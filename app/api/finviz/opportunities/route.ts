...
export async function GET() {
    try {
        ...
        console.log('Finviz Volume API route: Starting fetch at:', new Date().toISOString());
        ...
        console.log('Finviz Volume API route: Fetch completed at:', new Date().toISOString());
        console.log('Finviz Volume API route: Fetch duration:', endTime - startTime, 'ms');
        clearTimeout(timeoutId);
        
        console.log('Finviz Volume API route response status:', response.status);
        ...
        const transformedData = {
            high_volume_stocks: rawData || [],
            total_high_volume_stocks: rawData ? rawData.length : 0,
        ...
        const errorMessage = error instanceof Error 
            ? `Failed to load high volume stocks data: ${error.message}` 
            : 'Failed to load high volume stocks data';
